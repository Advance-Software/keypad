...
##################################################
#### Нстроем синтаксис для удобства
################################################## 
Function TSynHighlighter($self, $B) {
	...
	#######################
	## Присвоим имя или это будет все зазря
	#######################
	if(!empty($s) && !empty($B)) {
		$s->name = $B[1];
	}
...
################################################## 
#### функция для применения синтаксиса как парметр highlighterне  не принимает значения и всегда 0
################################################## 
...
##################################################
#### Теперь для mainMenu или обычного легко применения создаем функции быстрой смены синтаксиса
##################################################
Function SHTML() {
...
$data = "tmemo$x";
    ...
    highlighterSynEdit($data, 'Form1.SynHTML');
}
...
$data = "tmemo$x";
    ...
    highlighterSynEdit($data, 'Form1.SynPHP');
}
...
$data = "tmemo$x";
    ...
    highlighterSynEdit($data, 'Form1.SynGeneral');
}
...
$data = "tmemo$x";
    ...
    highlighterSynEdit($data, 'Form1.SynCpp');
}
...
$data = "tmemo$x";
    ...
    highlighterSynEdit($data, 'Form1.SynCss');
}
...
$data = "tmemo$x";
    ...
    highlighterSynEdit($data, 'Form1.SynSQL');
}
...
$data = "tmemo$x";
    ...
    highlighterSynEdit($data, 'Form1.SynJScript');
}
...
$data = "tmemo$x";
    ...
    highlighterSynEdit($data, 'Form1.SynXML');
}
